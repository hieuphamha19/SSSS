    ...
    for model in models:
        for m in model.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight)  # Updated function
...
class ResEncoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.relu = nn.ReLU(inplace=False)  # Chuyển thành False để tránh lỗi in-place
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)  # Đảm bảo relu không sử dụng in-place
        out = self.conv2(out)
        out = self.bn2(out)
        out += residual  # Hãy đảm bảo rằng residual không bị thay đổi in-place
        out = self.relu(out)  # Một lần nữa, sử dụng relu không in-place
        return out
...
class CSNet(nn.Module):
    ...
    def forward(self, x):
        ...
        final = torch.sigmoid(final)  # Sigmoid for binary classification or segmentation
        return final
